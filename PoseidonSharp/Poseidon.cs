...
namespace PoseidonSharp
    {
        public class Poseidon
        {
            ...
            public BigInteger CalculatePoseidonHash(BigInteger[] inputs, bool chained = false, bool trace = false)
            {
                ...
                foreach (BigInteger bigInt in ConstantsC)
                {
                    ...
                    if (k < halfF || k >= (halfF + NRoundsP))
                    {
                        ...
                        {
                            state[j] = ModPow(state[j], ReducedExponent, SNARK_SCALAR_FIELD);
                        });
                    }
                    else
                    {
                        state[0] = ModPow(state[0], ReducedExponent, SNARK_SCALAR_FIELD);
                    }
            ...
            public static BigInteger ModPow(BigInteger b, BigInteger exp, BigInteger n)
            {
                if (exp == 0) return 1;
                BigInteger r = 1;
                BigInteger x = b % n;
