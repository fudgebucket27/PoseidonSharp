...
public static class LoopringL2KeyGenerator
{
    ...
    {
        return RipKeyAppart((_seed, address), skipPublicKeyCalculation);
    }

    public static (string secretKey, string ethAddress, string publicKeyX, string publicKeyY) GenerateL2KeyDetails(string seed, string ethAddress, bool skipPublicKeyCalculation)
    {
        var keyDetails = GetL2KeyDetails(seed, ethAddress, skipPublicKeyCalculation);
        // We're only interested in the secret key for signing packages. Which ironically is the simplest one to get...
        return (keyDetails.secretKey, keyDetails.ethAddress, keyDetails.publicKeyX, keyDetails.publicKeyY);
    }
    ...
    private static BigInteger[] addPoint(BigInteger[] a, BigInteger[] b, BigInteger p)
    {
    ...
    private static BigInteger Pmul(BigInteger a, BigInteger b, BigInteger p)
    {
        return BigInteger.Remainder(BigInteger.Multiply(a, b), p);
    }

    private static BigInteger Psub(BigInteger a, BigInteger b, BigInteger p)
    {
        return (a >= b) ? BigInteger.Subtract(a, b) : BigInteger.Add(BigInteger.Subtract(p, b), a);
    }

    private static BigInteger Padd(BigInteger a, BigInteger b, BigInteger p)
    {
        var res = BigInteger.Add(a, b);
        return (res >= p ? res - p : res);
    }

    private static BigInteger Pdiv(BigInteger a, BigInteger b, BigInteger p)
    {
