...
namespace PoseidonSharp
{
    public static class SHA256Helper
    {
        ...
        public static BigInteger CalculateSHA256HashNumber(string requestText)
        {
            ...
            StringBuilder sha256HashBuilder = new StringBuilder(sha256HashBytes.Length * 2);
            foreach (byte x in sha256HashBytes)
            {
                sha256HashBuilder.AppendFormat("{0:x2}", x);
            }
            string sha256HashString = sha256HashBuilder.ToString();
